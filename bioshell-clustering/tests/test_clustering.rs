...
use bioshell_clustering::{expectation_maximization, EuclideanPoints, Optics};

use bioshell_statistics::{
    Distribution, MultiNormalDistribution, NormalDistribution, OnlineMultivariateStatistics,
};
...
fn test_expectation_maximization() {
    ...
    let mut normals = vec![
        NormalDistribution::new(0.0, 0.5),
    ...
    for i in 0..n_dist {
        for j in 0..n_data_1 {
            normals[i].sample(&mut rng, &mut sample[i * n_data_1 + j]);
        }
...
fn Optics_clustering_Gaussian_data() {
    ...
    let opt_clust = Optics::new(0.5, 5, Box::new(EuclideanPoints::new(data.clone())));
...
fn test_Optics() {
    ...
    let opt_clust = Optics::new(2.5, 4, Box::new(EuclideanPoints::new(data.clone())));
    let cluster_size: Vec<usize> = opt_clust.clusters().iter().map(|r| r.len()).collect();
