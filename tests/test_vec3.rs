...
fn vec3_arithmetic() {
    ...
    assert!(f32::abs(v0.x - 3.45)  < 1e-5);
    assert!(f32::abs(v0.y - 3.45)  < 1e-5);
    assert!(f32::abs(v0.z - 3.45)  < 1e-5);

    v0.res_type = 1;
    v0.atom_type = 3;
    v0.chain_id = 10;
    v0.normalize();
    assert!(f32::abs(v0.length() - 1.0)  < 1e-5);
    let v1 = v0.clone();
    assert_eq!(v1.res_type, 1);
    assert_eq!(v1.atom_type, 3);
    assert_eq!(v1.chain_id, 10);
    assert!(f32::abs(Vec3::dot(&v0, &v1) - 1.0) < 1e-5);
    v0.mul(-1.0);
    assert!(f32::abs(Vec3::dot(&v0, &v1) + 1.0) < 1e-5);
}
...
fn calculate_planar_angles() {
    ...
    let b = Vec3::new(0.0, d * (3.0 as f32).sqrt(), 0.0);
