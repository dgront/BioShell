...
impl OnlineMultivariateStatistics {
    ...
    pub fn accumulate(&mut self, d: &[f64]) {
        ...
        if self.count == 0 {
            // --- if this is the very first observation...
            for i in 0..self.dim {
        ...
        for i in 0..self.dim {
            ...
            for j in i + 1..self.dim {
                // --- here we update the covariance terms
    ...
    ///  # Arguments
    /// * `id` - index of the coordinate i.e. dimension (starts form 0)
    pub fn var(&self, id: usize) -> f64 {
        self.m2[id] / (self.count as f64 - 1.0)
    }
    ...
    pub fn covar(&self, i: usize, j: usize) -> f64 {
        self.cov[i][j] / (self.count as f64 - 1.0)
    }
}
