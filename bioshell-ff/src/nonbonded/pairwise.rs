...
impl PairwiseNonbonded for SimpleContact {
    fn energy_for_distance_squared(&self, d2: f64) -> f64 {
        ...
        if d2 < self.r_c_sq {
            return self.repulsion_energy as f64;
        }
        if d2 > self.r_from_sq {
            return self.contact_energy as f64;
        } else {
            return 0.0;
        }
    }
...
impl LennardJonesHomogenic {
    pub fn new(epsilon: f64, sigma: f64, cutoff: f64) -> LennardJonesHomogenic {
        LennardJonesHomogenic {
            epsilon,
            sigma,
            cutoff,
            sigma_sq: sigma * sigma,
            cutoff_sq: cutoff * cutoff,
            epsilon_four: 4.0 * epsilon,
        }
    }
}
