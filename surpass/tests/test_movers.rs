...
mod test_movers {
    ...
    fn make_predefined_hinge_move_N(move_size: usize) {
        ...
        let mover: HingeMove = HingeMove::new(move_size,std::f64::consts::PI / 2.0, std::f64::consts::PI / 2.0);
        let mut mp = MoveProposal::new(move_size);
        let v0_before = model.ca_to_vec3(0);
        let v1_before = model.ca_to_vec3(1);
        let last_before = model.ca_to_vec3(move_size+1);
        mover.compute_move(&model, 1, std::f64::consts::PI / 2.0, &mut mp);
        mp.apply(&mut model);
        let v0_after = model.ca_to_vec3(0);
        let v1_after = model.ca_to_vec3(1);
        let last_after = model.ca_to_vec3(move_size+1);
    ...
    fn make_predefined_tail_move() {
        ...
        let tail: TailMove = TailMove::new(1, std::f64::consts::PI / 2.0, std::f64::consts::PI / 2.0);
        let mut tail_prop = MoveProposal::new(1);
        // ---------- coordinates of atoms before any move
        let n_before = model.ca_to_vec3(0);
        let c_before = model.ca_to_vec3(3);
        // ---------- coordinates of preceding atoms to check bond lengths
        let n_preceding = model.ca_to_vec3(1);
        let c_preceding = model.ca_to_vec3(2);
        assert_delta!(n_preceding.distance_to(&n_before), 3.8, 0.001);
        assert_delta!(c_preceding.distance_to(&c_before), 3.8, 0.001);
        ...
        assert!(n_before.distance_to(&n_after) > 0.5, "N-teminal didn't move further than 0.5A which was expected!");
        assert_delta!(n_preceding.distance_to(&n_after), 3.8, 0.001);
        ...
        tail_prop.apply(&mut model);
        let c_after = model.ca_to_vec3(3);
        assert!(c_before.distance_to(&c_after) > 0.5, "C-teminal didn't move further than 0.5A which was expected!");
    }
    ...
    #[allow(non_snake_case)]
    fn make_predefined_tail_move_N(move_size: usize) {
        const N_RES: usize = 10;
        let mut model = extended_chain(N_RES, 1000.0);
        let tail: TailMove = TailMove::new(move_size, std::f64::consts::PI / 2.0, std::f64::consts::PI / 2.0);
        let mut tail_prop = MoveProposal::new(move_size);
        let mut backup: MoveProposal = MoveProposal::new(move_size);
        ...
        backup.first_moved_pos = N_RES-move_size;
        backup.backup(&model);
        tail.compute_move(&mut model, 0, MovedTermini::CTerminal, 1.57, &mut tail_prop);
        tail_prop.apply(&mut model);
        let offset = N_RES-move_size;
        for i in 0..move_size {
    ...
    fn move_chain() {
        ...
        let hinge: HingeMove = HingeMove::new(4, std::f64::consts::PI / 2.0, std::f64::consts::PI / 2.0);
        let tail: TailMove = TailMove::new(1, std::f64::consts::PI / 2.0, std::f64::consts::PI / 2.0);
        let mut hinge_prop: MoveProposal = MoveProposal::new(4);
        let mut tail_prop: MoveProposal = MoveProposal::new(1);
